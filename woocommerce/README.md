...
3. **Produkte anlegen**: Die Leistungen aus `shop.html` (Diagnose, Reverse-Engineering-Analyse, Steuergeräte-Codierung, SFD-/SFD2-Freischaltung, VCDS-/VCP-/SFD-SFD2-UNECE-Schulungen) als WooCommerce-Produkte übertragen — mit den dann finalen echten Preisen statt der aktuellen `[PREIS ERGÄNZEN]`-Platzhalter.
...
6. **Hosting-Strategie festlegen**: Entweder komplette Migration der Seite auf den neuen WordPress-Server, oder Betrieb parallel unter einer Subdomain (z. B. `shop.lhnext.de`), während die Hauptseite weiter statisch auf GitHub Pages bleibt. Bei einer Subdomain-Lösung braucht es einen eigenen Domain-/DNS-Eintrag (GitHub Pages allein reicht dafür nicht).
